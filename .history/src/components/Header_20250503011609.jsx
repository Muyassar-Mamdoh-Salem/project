...
const Header = () => {
  ...
  // الحصول على عدد المنتجات في السلة من الـ Redux
  const count = useSelector((state) => state.Ecommerce?.products.length || 0); // عدد المنتجات في السلة
          ...
          <div className="flex justify-end items-center space-x-4">
            ...
            <div className="hidden md:flex space-x-4 text-xl text-gray-700">
              ...
              <HashLink to="/cart" className="relative inline-block">
                {/* عرض العداد على أيقونة السلة */}
                {count > 0 && (
                  <span className="absolute -top-2 -right-2 bg-red-500 text-white text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center">{count}</span>
                )}
