import React, { useState } from "react";
import { HashLink } from 'react-router-hash-link';
import { FaSearch, FaUser, FaShoppingCart, FaBars, FaSignOutAlt } from "react-icons/fa";
import { MdFavoriteBorder } from "react-icons/md";
import { IoMdClose } from "react-icons/io";
import { useDispatch, useSelector } from "react-redux";
import { getAuth, signOut } from "firebase/auth";
import { Logoutuser } from "../Redux/appSlice";

const iconSize = 24; // حجم موحد لكل الأيقونات

const Header = () => {
  const UserInfo = useSelector((state) => state.app.UserInfo);
  const dispatch = useDispatch();

  const [openBar, setOpenBar] = useState(false);

  const totalItems = useSelector((state) =>
    state.app.products.reduce((total, item) => total + item.quantity, 0)
  );

  const handleLogout = () => {
    const auth = getAuth();
    signOut(auth).then(() => {
      console.log("Sign-out successful.");
      dispatch(Logoutuser());
    }).catch((error) => {
      console.log("An error happened", error);
    });
  }

  return (
    <>
      <header className="bg-gray-200 shadow-md fixed top-0 left-0 right-0 z-30">
        <div className="container mx-auto grid grid-cols-1 md:grid-cols-3 items-center gap-4 px-4 py-3">
          {/* Logo */}
          <div className="flex justify-center md:justify-start">
            <h2 className="text-2xl font-bold">
              <span className="text-blue-600">Shop</span> Ease
            </h2>
          </div>

          {/* Search bar - حركته ناحية الشمال قليلاً */}
      {/* Sidebar Menu - Mobile only */}
<div
  className={`fixed top-0 right-0 w-[220px] h-[100vh] transform ${openBar ? "translate-x-0" : "translate-x-full"
    } transition-transform duration-500 ease-in-out z-50 bg-gradient-to-b from-blue-900 via-blue-800 to-gray-900 md:hidden`}
>
  <div
    className="absolute top-4 left-3 text-white cursor-pointer"
    onClick={() => setOpenBar(false)}
  >
    <IoMdClose size={iconSize} />
  </div>

  {/* Sidebar Icons + User Info - عمود بدلاً من صف */}
  <div className="flex flex-col  items-center space-y-6 py-8 text-white border-b border-gray-600 px-6">
    {UserInfo ? (import React, { useState } from "react";
import { HashLink } from 'react-router-hash-link';
import { FaSearch, FaUser, FaShoppingCart, FaBars, FaSignOutAlt } from "react-icons/fa";
import { MdFavoriteBorder } from "react-icons/md";
import { IoMdClose } from "react-icons/io";
import { useDispatch, useSelector } from "react-redux";
import { getAuth, signOut } from "firebase/auth";
import { Logoutuser } from "../Redux/appSlice";

const iconSize = 24; // حجم موحد لكل الأيقونات

const Header = () => {
  const UserInfo = useSelector((state) => state.app.UserInfo);
  const dispatch = useDispatch();

  const [openBar, setOpenBar] = useState(false);

  const totalItems = useSelector((state) =>
    state.app.products.reduce((total, item) => total + item.quantity, 0)
  );

  const handleLogout = () => {
    const auth = getAuth();
    signOut(auth).then(() => {
      console.log("Sign-out successful.");
      dispatch(Logoutuser());
    }).catch((error) => {
      console.log("An error happened", error);
    });
  };

  // دالة ممكن تستخدمها للتنقل أو تفتح نافذة بحث
  const handleSearchClick = () => {
    alert("Search button clicked!"); // مثال، ممكن تستبدلها بالوظيفة اللي تحبها
  };

  return (
    <>
      <header className="bg-gray-200 shadow-md fixed top-0 left-0 right-0 z-30">
        <div className="container mx-auto grid grid-cols-1 md:grid-cols-3 items-center gap-4 px-4 py-3">
          {/* Logo */}
          <div className="flex justify-center md:justify-start">
            <h2 className="text-2xl font-bold">
              <span className="text-blue-600">Shop</span> Ease
            </h2>
          </div>

          {/* Search bar - حركته ناحية الشمال قليلاً */}
          {/* Sidebar Menu - Mobile only */}
          <div
            className={`fixed top-0 right-0 w-[220px] h-[100vh] transform ${
              openBar ? "translate-x-0" : "translate-x-full"
            } transition-transform duration-500 ease-in-out z-50 bg-gradient-to-b from-blue-900 via-blue-800 to-gray-900 md:hidden`}
          >
            <div
              className="absolute top-4 left-3 text-white cursor-pointer"
              onClick={() => setOpenBar(false)}
            >
              <IoMdClose size={iconSize} />
            </div>

            {/* Sidebar Icons + User Info - عمود بدلاً من صف */}
            <div className="flex flex-col  items-center space-y-6 py-8 text-white border-b border-gray-600 px-6">
              {UserInfo ? (
                <span className="text-sm font-semibold">{UserInfo.userName}</span>
              ) : (
                <HashLink to="/regpage" onClick={() => setOpenBar(false)}>
                  <FaUser
                    className="active:text-yellow-400 cursor-pointer transition duration-300"
                    size={iconSize}
                  />
                </HashLink>
              )}

              <button
                onClick={() => {
                  alert("Search clicked!");
                  setOpenBar(false);
                }}
                className="active:text-yellow-400 cursor-pointer transition duration-300"
                aria-label="Search"
              >
                <FaSearch size={iconSize} />
              </button>

              <HashLink to="/cart" onClick={() => setOpenBar(false)} className="relative">
                <span className="absolute -top-2 -right-2 bg-red-500 text-white text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center">
                  {totalItems}
                </span>
                <FaShoppingCart
                  className="active:text-yellow-400 cursor-pointer transition duration-300"
                  size={iconSize}
                />
              </HashLink>

              {UserInfo && (
                <button
                  onClick={handleLogout}
                  className="flex items-center space-x-2 text-sm font-semibold text-white hover:text-yellow-400 transition duration-300"
                >
                  <FaSignOutAlt size={iconSize} />
                  <span>Logout</span>
                </button>
              )}

              <MdFavoriteBorder
                className="active:text-yellow-400 cursor-pointer transition duration-300"
                size={iconSize}
              />
            </div>

            {/* Sidebar Navigation */}
            <nav className="mt-10 text-white flex flex-col space-y-6 px-6 text-lg">
              <HashLink
                smooth
                to="/#top"
                onClick={() => setOpenBar(false)}
                className="hover:text-yellow-400 transition duration-300"
              >
                Home
              </HashLink>
              <HashLink
                to="/shop"
                onClick={() => setOpenBar(false)}
                className="hover:text-yellow-400 transition duration-300"
              >
                Shop
              </HashLink>
              <HashLink
                to="/categories"
                onClick={() => setOpenBar(false)}
                className="hover:text-yellow-400 transition duration-300"
              >
                Categories
              </HashLink>
              <HashLink
                to="/about"
                onClick={() => setOpenBar(false)}
                className="hover:text-yellow-400 transition duration-300"
              >
                About
              </HashLink>
            </nav>
          </div>

          {/* Desktop icons and menu */}
          <div className="flex justify-end items-center space-x-4">
            {/* Navigation Links */}
            <nav className="hidden md:flex space-x-6 text-lg">
              <HashLink
                smooth
                to="/#top"
                onClick={() => setOpenBar(false)}
                className="hover:text-blue-500 transition duration-300"
              >
                Home
              </HashLink>
              <HashLink
                to="/shop"
                className="hover:text-blue-500 transition duration-300"
              >
                Shop
              </HashLink>
              <HashLink
                to="/categories"
                className="hover:text-blue-500 transition duration-300"
              >
                Categories
              </HashLink>
              <HashLink
                to="/about"
                className="hover:text-blue-500 transition duration-300"
              >
                About
              </HashLink>
            </nav>

            {/* Desktop Icons */}
            <div className="hidden md:flex items-center space-x-4 text-gray-700">
              {UserInfo ? (
                <span className="text-sm font-semibold text-gray-800 mr-2">
                  Hi, {UserInfo.userName}
                </span>
              ) : (
                <HashLink to="/regpage">
                  <FaUser
                    className="hover:text-blue-500 cursor-pointer transition duration-300"
                    size={iconSize}
                  />
                </HashLink>
              )}

              <button
                onClick={handleSearchClick}
                aria-label="Search"
                className="hover:text-blue-500 cursor-pointer transition duration-300"
              >
                <FaSearch size={iconSize} />
              </button>

              <HashLink to="/cart" className="relative inline-block">
                <span className="absolute -top-2 -right-2 bg-red-500 text-white text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center">
                  {totalItems}
                </span>
                <FaShoppingCart
                  className="hover:text-blue-500 cursor-pointer transition duration-300"
                  size={iconSize}
                />
              </HashLink>

              <MdFavoriteBorder
                className="hover:text-blue-500 cursor-pointer transition duration-300"
                size={iconSize}
              />
            </div>

            {/* Mobile Menu Button */}
            <div
              className="md:hidden cursor-pointer text-gray-800"
              onClick={() => setOpenBar(!openBar)}
            >
              {!openBar ? (
                <FaBars size={iconSize} />
              ) : (
                <IoMdClose size={iconSize} />
              )}
            </div>
          </div>
        </div>
      </header>

      {/* Sidebar Menu - Mobile only */}
      <div
        className={`fixed top-0 right-0 w-[220px] h-[100vh] transform ${
          openBar ? "translate-x-0" : "translate-x-full"
        } transition-transform duration-500 ease-in-out z-50 bg-gradient-to-b from-blue-900 via-blue-800 to-gray-900 md:hidden`}
      >
        <div
          className="absolute top-4 left-3 text-white cursor-pointer"
          onClick={() => setOpenBar(false)}
        >
          <IoMdClose size={iconSize} />
        </div>

        {/* Sidebar Icons */}
        <div className="flex items-center justify-around py-5 text-white mt-12 border-b border-gray-600">
          {UserInfo ? (
            <span className="text-sm font-semibold">{UserInfo.userName}</span>
          ) : (
            <HashLink to="/regpage" onClick={() => setOpenBar(false)}>
              <FaUser
                className="active:text-yellow-400 cursor-pointer transition duration-300"
                size={iconSize}
              />
            </HashLink>
          )}

          <button
            onClick={() => {
              alert("Search clicked!");
              setOpenBar(false);
            }}
            aria-label="Search"
            className="active:text-yellow-400 cursor-pointer transition duration-300"
          >
            <FaSearch size={iconSize} />
          </button>

          <HashLink to="/cart" onClick={() => setOpenBar(false)} className="relative">
            <span className="absolute -top-2 -right-2 bg-red-500 text-white text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center">
              {totalItems}
            </span>
            <FaShoppingCart
              className="active:text-yellow-400 cursor-pointer transition duration-300"
              size={iconSize}
            />
          </HashLink>

          {UserInfo && (
            <span className="flex items-center space-x-2">
              <FaSignOutAlt className="text-blue-400 cursor-pointer" size={iconSize} />
              <button
                onClick={handleLogout}
                className="text-sm font-semibold text-white hover:text-yellow-400 transition duration-300"
              >
                Logout
              </button>
            </span>
          )}

          <MdFavoriteBorder
            className="active:text-yellow-400 cursor-pointer transition duration-300"
            size={iconSize}
          />
        </div>

        {/* Sidebar Navigation */}
        <nav className="mt-10 text-white flex flex-col space-y-6 px-6 text-lg">
          <HashLink
            smooth
            to="/#top"
            onClick={() => setOpenBar(false)}
            className="hover:text-yellow-400 transition duration-300"
          >
            Home
          </HashLink>
          <HashLink
            to="/shop"
            onClick={() => setOpenBar(false)}
            className="hover:text-yellow-400 transition duration-300"
          >
            Shop
          </HashLink>
          <HashLink
            to="/categories"
            onClick={() => setOpenBar(false)}
            className="hover:text-yellow-400 transition duration-300"
          >
            Categories
          </HashLink>
          <HashLink
            to="/about"
            onClick={() => setOpenBar(false)}
            className="hover:text-yellow-400 transition duration-300"
          >
            About
          </HashLink>
        </nav>
      </div>
    </>
  );
};

export default Header;

      <span className="text-sm font-semibold">{UserInfo.userName}</span>
    ) : (
      <HashLink to="/regpage" onClick={() => setOpenBar(false)}>
        <FaUser
          className="active:text-yellow-400 cursor-pointer transition duration-300"
          size={iconSize}
        />
      </HashLink>
    )}

    <HashLink
      to="/cart"
      onClick={() => setOpenBar(false)}
      className="relative"
    >
      <span className="absolute -top-2 -right-2 bg-red-500 text-white text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center">
        {totalItems}
      </span>
      <FaShoppingCart
        className="active:text-yellow-400 cursor-pointer transition duration-300"
        size={iconSize}
      />
    </HashLink>

    {UserInfo && (
      <button
        onClick={handleLogout}
        className="flex items-center space-x-2 text-sm font-semibold text-white hover:text-yellow-400 transition duration-300"
      >
        <FaSignOutAlt size={iconSize} />
        <span>Logout</span>
      </button>
    )}

    <MdFavoriteBorder
      className="active:text-yellow-400 cursor-pointer transition duration-300"
      size={iconSize}
    />
  </div>

  {/* Sidebar Navigation */}
  <nav className="mt-10 text-white flex flex-col space-y-6 px-6 text-lg">
    <HashLink
      smooth
      to="/#top"
      onClick={() => setOpenBar(false)}
      className="hover:text-yellow-400 transition duration-300"
    >
      Home
    </HashLink>
    <HashLink
      to="/shop"
      onClick={() => setOpenBar(false)}
      className="hover:text-yellow-400 transition duration-300"
    >
      Shop
    </HashLink>
    <HashLink
      to="/categories"
      onClick={() => setOpenBar(false)}
      className="hover:text-yellow-400 transition duration-300"
    >
      Categories
    </HashLink>
    <HashLink
      to="/about"
      onClick={() => setOpenBar(false)}
      className="hover:text-yellow-400 transition duration-300"
    >
      About
    </HashLink>
  </nav>
</div>


          {/* Desktop icons and menu */}
          <div className="flex justify-end items-center space-x-4">
            {/* Navigation Links */}
            <nav className="hidden md:flex space-x-6 text-lg">
              <HashLink smooth to="/#top" onClick={() => setOpenBar(false)} className="hover:text-blue-500 transition duration-300">Home</HashLink>
              <HashLink to="/shop" className="hover:text-blue-500 transition duration-300">Shop</HashLink>
              <HashLink to="/categories" className="hover:text-blue-500 transition duration-300">Categories</HashLink>
              <HashLink to="/about" className="hover:text-blue-500 transition duration-300">About</HashLink>
            </nav>

            {/* Desktop Icons */}
            <div className="hidden md:flex items-center space-x-4 text-gray-700">
              {UserInfo ? (
                <span className="text-sm font-semibold text-gray-800 mr-2">
                  Hi, {UserInfo.userName}
                </span>
              ) : (
                <HashLink to="/regpage">
                  <FaUser className="hover:text-blue-500 cursor-pointer transition duration-300" size={iconSize} />
                </HashLink>
              )}

              <HashLink to="/cart" className="relative inline-block">
                <span className="absolute -top-2 -right-2 bg-red-500 text-white text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center">
                  {totalItems}
                </span>
                <FaShoppingCart className="hover:text-blue-500 cursor-pointer transition duration-300" size={iconSize} />
              </HashLink>

              <MdFavoriteBorder className="hover:text-blue-500 cursor-pointer transition duration-300" size={iconSize} />
            </div>

            {/* Mobile Menu Button */}
            <div
              className="md:hidden cursor-pointer text-gray-800"
              onClick={() => setOpenBar(!openBar)}
            >
              {!openBar ? <FaBars size={iconSize} /> : <IoMdClose size={iconSize} />}
            </div>
          </div>
        </div>
      </header>

      {/* Sidebar Menu - Mobile only */}
      <div
        className={`fixed top-0 right-0 w-[220px] h-[100vh] transform ${openBar ? "translate-x-0" : "translate-x-full"
          } transition-transform duration-500 ease-in-out z-50 bg-gradient-to-b from-blue-900 via-blue-800 to-gray-900 md:hidden`}
      >
        <div
          className="absolute top-4 left-3 text-white cursor-pointer"
          onClick={() => setOpenBar(false)}
        >
          <IoMdClose size={iconSize} />
        </div>

        {/* Sidebar Icons */}
        <div className="flex items-center justify-around py-5 text-white mt-12 border-b border-gray-600">
          {UserInfo ? (
            <span className="text-sm font-semibold">
              {UserInfo.userName}
            </span>
          ) : (
            <HashLink to="/regpage" onClick={() => setOpenBar(false)}>
              <FaUser className="active:text-yellow-400 cursor-pointer transition duration-300" size={iconSize} />
            </HashLink>
          )}

          <HashLink to="/cart" onClick={() => setOpenBar(false)} className="relative">
            <span className="absolute -top-2 -right-2 bg-red-500 text-white text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center">
              {totalItems}
            </span>
            <FaShoppingCart className="active:text-yellow-400 cursor-pointer transition duration-300" size={iconSize} />
          </HashLink>

          {UserInfo && (
            <span className="flex items-center space-x-2">
              <FaSignOutAlt className="text-blue-400 cursor-pointer" size={iconSize} />
              <button onClick={handleLogout} className="text-sm font-semibold text-white hover:text-yellow-400 transition duration-300">
                Logout
              </button>
            </span>
          )}

          <MdFavoriteBorder className="active:text-yellow-400 cursor-pointer transition duration-300" size={iconSize} />
        </div>

        {/* Sidebar Navigation */}
        <nav className="mt-10 text-white flex flex-col space-y-6 px-6 text-lg">
          <HashLink smooth to="/#top" onClick={() => setOpenBar(false)} className="hover:text-yellow-400 transition duration-300">Home</HashLink>
          <HashLink to="/shop" onClick={() => setOpenBar(false)} className="hover:text-yellow-400 transition duration-300">Shop</HashLink>
          <HashLink to="/categories" onClick={() => setOpenBar(false)} className="hover:text-yellow-400 transition duration-300">Categories</HashLink>
          <HashLink to="/about" onClick={() => setOpenBar(false)} className="hover:text-yellow-400 transition duration-300">About</HashLink>
        </nav>
      </div>
    </>
  );
};

export default Header;
