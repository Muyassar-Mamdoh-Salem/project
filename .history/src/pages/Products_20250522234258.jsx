...
const Products = () => {
  ...
  // حالة محلية لتتبع أي منتج تم إضافته مؤخرًا للمفضلة (لتغيير اللون مؤقتاً)
  const [addedToFavId, setAddedToFavId] = useState(null);

  const handleAddToFavorites = (product) => {
    // إذا المنتج مش موجود في المفضلة نضيفه
    if (!favorites.find(item => item.id === product.id)) {
      dispatch(addToFavorites(product));
      setAddedToFavId(product.id);

      // إعادة تعيين الحالة بعد 1.5 ثانية (لإزالة تأثير التغيير)
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      ...
      >
        {products.map((product) => (
          ...
          >
            {/* أيقونة المفضلة تظهر عند hover على البطاقة باستخدام tailwind group-hover */}
            <button
              onClick={() => handleAddToFavorites(product)}
              aria-label="Add to favorites"
              title="Add to favorites"
              className={`absolute top-3 right-3 transition-opacity duration-300
                ${favorites.find(item => item.id === product.id) ? 'text-red-500 opacity-100' : 'text-gray-400 opacity-0 group-hover:opacity-100'}
                bg-white rounded-full p-1 shadow-md sm-hover:scale-110
              `}
            >
