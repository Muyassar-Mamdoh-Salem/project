  ...
  const handleLogin = (e) => {
    ...
    signInWithEmailAndPassword(auth, email, password)
      .then((userCredential) => {
        ...
        // إرسال بيانات المستخدم إلى Redux
        dispatch(
          setUser({
            __id: user.uid,
            userName: user.displayName || user.email, // لو ما فيه displayName خلي البريد
            email: user.email,
          })
        );
        // التنقل إلى الصفحة الرئيسية مباشرة بعد تسجيل الدخول
        navigate("/");
      })
      .catch((error) => {
