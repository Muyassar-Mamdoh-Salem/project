  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8 px-2 sm:px-4 w-full overflow-x-hidden">
      ...
      <div className="max-w-[100vw] mx-auto">
        ...
        {/* شبكة المنتجات */}
        <motion.div
          className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 px-2"
          initial="hidden"
          animate="visible"
          variants={{
            hidden: { opacity: 0 },
        ...
        >
          {products.map((product) => (
            <motion.div
              key={product.id}
              className="relative bg-white rounded-lg shadow-sm p-3 flex flex-col justify-between group  sm:w-[250px]"
              variants={{
